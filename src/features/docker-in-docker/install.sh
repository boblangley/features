        ...
        rhel)
            if [ "${USE_MOBY}" = "true" ]; then
                ...
                if [ "${DOCKER_DASH_COMPOSE_VERSION}" != "none" ]; then
                    ${PKG_MGR_CMD} -y install moby-compose || { err "Moby was explicitly requested, but moby-compose is unavailable for ${ID} ${VERSION_CODENAME} (${architecture})."; exit 1; }
                fi
            else
...
if [ ! -x /init ] || [ ! -d "${S6_RC_DIR}" ] || [ ! -d "${S6_USER_BUNDLE}" ]; then
    err "docker-in-docker requires an s6-overlay base image with /init as its entrypoint."
    exit 1
fi

prepare_dir="${S6_RC_DIR}/docker-in-docker-prepare"
containerd_dir="${S6_RC_DIR}/docker-in-docker-containerd"
dockerd_dir="${S6_RC_DIR}/docker-in-docker"
mkdir -p \
    "${prepare_dir}" \
    "${containerd_dir}/dependencies.d" \
    "${dockerd_dir}/dependencies.d"
printf 'oneshot\n' > "${prepare_dir}/type"
printf 'longrun\n' > "${containerd_dir}/type"
printf 'longrun\n' > "${dockerd_dir}/type"
touch \
    "${containerd_dir}/dependencies.d/docker-in-docker-prepare" \
    "${dockerd_dir}/dependencies.d/docker-in-docker-containerd" \
    "${S6_USER_BUNDLE}/docker-in-docker"

cat > "${prepare_dir}/prepare" << EOF
#!/command/with-contenv bash
set -euo pipefail

find /run /var/run -iname 'docker*.pid' -delete || true
find /run /var/run -iname 'container*.pid' -delete || true
mkdir -p /run/containerd

if [ '${IPTABLES_SWITCH_AT_RUNTIME}' = true ] \\
    ...
    update-alternatives --set iptables /usr/sbin/iptables-legacy || true
    update-alternatives --set ip6tables /usr/sbin/ip6tables-legacy || true
elif [ '${IPTABLES_SWITCH_AT_RUNTIME}' = true ] \\
    && command -v iptables-nft >/dev/null \\
...
if [ -d /sys/kernel/security ] && ! mountpoint -q /sys/kernel/security; then
    mount -t securityfs none /sys/kernel/security || echo "docker-in-docker: unable to mount securityfs" >&2
fi
...
EOF
chmod 0755 "${prepare_dir}/prepare"
printf '%s\n' "${prepare_dir}/prepare" > "${prepare_dir}/up"

cat > "${containerd_dir}/run" << 'EOF'
#!/command/with-contenv bash
set -euo pipefail
for candidate in /usr/local/bin/containerd /usr/bin/containerd /usr/sbin/containerd; do
    if [ -x "${candidate}" ]; then
        exec "${candidate}" --config /etc/containerd/config.toml
    fi
done
echo "docker-in-docker: containerd executable not found" >&2
exit 127
EOF
chmod 0755 "${containerd_dir}/run"

cat > "${dockerd_dir}/run" << EOF
#!/command/with-contenv bash
set -euo pipefail
