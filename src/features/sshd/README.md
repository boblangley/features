...
Adds an SSH server supervised as a native s6-overlay service.
...
The image must use s6-overlay 3 with `/init` as its entrypoint. Set `"overrideCommand": false` in `devcontainer.json` so s6 remains PID 1.
