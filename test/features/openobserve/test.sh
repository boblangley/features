...
check "OpenObserve default root user email is set" grep -Fq 'ZO_ROOT_USER_EMAIL="${ZO_ROOT_USER_EMAIL:-root@example.com}"' /usr/local/bin/openobserve-service
check "OpenObserve default root user password is set" grep -Fq 'ZO_ROOT_USER_PASSWORD="${ZO_ROOT_USER_PASSWORD:-Complexpass#123}"' /usr/local/bin/openobserve-service
check "OpenObserve telemetry is disabled by default" grep -Fq 'ZO_TELEMETRY="${ZO_TELEMETRY:-false}"' /usr/local/bin/openobserve-service
